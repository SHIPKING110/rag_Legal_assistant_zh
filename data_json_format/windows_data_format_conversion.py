...
#-----------------------------
# 读取 doc 文件（Windows兼容版本）
#-----------------------------
def read_doc(path):
    try:
        # 先尝试使用 antiword（如果Windows用户安装了的话）
        text = subprocess.check_output(["antiword", path], stderr=subprocess.STDOUT)
        return text.decode("utf-8", errors="ignore")
    except Exception as e:
        # antiword 失败时，尝试使用 pywin32
        try:
            return read_doc_with_win32(path)
        except Exception as win32_error:
            print(f"所有方法读取失败: {path}, antiword错误: {e}, win32错误: {win32_error}")
            return None


def read_doc_with_win32(path):
    """使用Windows COM组件读取.doc文件"""
    try:
        ...
        raise Exception(f"Word COM读取失败: {str(e)}")
    finally:
        # 确保清理COM资源
        try:
